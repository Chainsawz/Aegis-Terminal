...
st.set_page_config(
    page_title="AEGIS TACTICAL v5.0", 
    layout="wide", 
    initial_sidebar_state="expanded"
)

# --- CSS: ESTÉTICA MILITAR DE ÉLITE ---
st.markdown("""
    <style>
...
def analyze_batch_intel(articles):
    ...
    try:
        # Usamos 1.5 Flash para estabilidad de cuota
        response = client.models.generate_content(model="gemini-1.5-flash", contents=prompt)
        res_text = response.text.strip().replace('```json', '').replace('```', '')
        analyzed = json.loads(res_text)
        return [{**articles[r['id']], **r} for r in analyzed]
    except Exception as e:
        st.sidebar.warning("⚠️ IA SATURADA: Usando feed analógico.")
        return []
...
# --- UI PRINCIPAL ---
st.markdown("<h1 style='color:#3b82f6;'>◤ AEGIS_BLACK_HORIZON_v5.0</h1>", unsafe_allow_html=True)

# Barra Lateral
st.sidebar.header("🕹️ CONTROL TÁCTICO")
...
m1.metric("NODOS_MAPA", len(st.session_state.memory), "IA_FILTERED")
m2.metric("SEÑALES_RAW", len(st.session_state.raw_feed))
m3.metric("STATUS", "SYNCED" if st.session_state.memory else "IDLE")

col_map, col_feed = st.columns([2.5, 1])

with col_map:
    # --- BLINDAJE DE MAPA (v5.0) ---
    m = folium.Map(
        location=[20, 0], 
        zoom_start=2.3, 
        tiles="CartoDB dark_matter",
        no_wrap=True,           # No repetición horizontal
        min_zoom=2.3,           # Bloqueo de alejamiento
        max_bounds=True,        # Muros del mundo activos
...
with col_feed:
    ...
    if st.session_state.memory:
        ...
        st.info("📡 Señales crudas detectadas (IA sin cuota)")
...
st.markdown("<p style='text-align:center; color:#1e293b; font-size:10px; margin-top:50px;'>AEGIS CORP © 2026 - SECURE TERMINAL v5.0</p>", unsafe_allow_html=True)
